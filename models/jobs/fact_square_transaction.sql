...
SELECT DISTINCT row_number() over (order by CREATED_AT DESC) AS TRANSACTION_ID, CAST(CREATED_AT AS DATE) AS date_of_transaction, NULL, REASON AS REFUND_REASON, LOCATION_ID, MERCHANT_ID, NULL, REFUNDED_MONEY:amount::NUMBER AS REFUNDED_AMOUNT, NULL, PAYMENT_ID, 'REFUND' AS TYPE
